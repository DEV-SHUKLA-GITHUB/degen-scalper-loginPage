...
const TradeDashboard = async () => {
  // const response = await fetch(__dirname, 'tokens.json'); // Fetch the file from the public folder
  // const data = await response.json();
  
  const arrayOfTokens= [8963586,8963842,10227202]
// console.log(arrayOfTokens,"lili");
// var instrumentToken;
      ...
      .then((data) => {
        for (const instrument of data.instruments) {
          if ((selected.name || selected) === instrument.name) {
            ...
            console.log(instrument.instrument_token , "sdfklskdf")
...
const socket = new WebSocket('ws://localhost:7000/instruments');
  ...
  socket.onopen = () => {
    ...
    //         // Now you have the array in the 'arrayOfTokens' variable
