...
const instrumentTokenRef = useRef(instrumentToken);
  ...
  //saving cache data in db (stopLoss, trailingStopLoss, stopLossTSL)
  useEffect(()=>{
    //post request to /updateDB
    fetch(`${API_URL}/updateCache`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        token: window.localStorage.getItem("token"),stopLoss, trailingStopLoss, stopLossTSL
      }),
    })
      .then((res) => res.json())
      .then((data) =>{
  ...
  }, [stopLoss, trailingStopLoss, stopLossTSL])
